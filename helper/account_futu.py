...
def _request_balance(symbol: str, market: str) -> None:
    csv_file = os.path.join(settings.RPT_DIR, f"balance_{symbol}.csv")
...
def _request_profit(symbol: str, market: str) -> None:
    csv_file = os.path.join(settings.RPT_DIR, f"profit_{symbol}.csv")
    if os.path.exists(csv_file):
...
def _request_cashflow(symbol: str, market: str) -> None:
    csv_file = os.path.join(settings.RPT_DIR, f"cashflow_{symbol}.csv")
    if os.path.exists(csv_file):

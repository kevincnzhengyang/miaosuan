...
LastEditTime: 2025-09-12 21:02:42
FilePath: /miaosuan2/helper/indicator_tools.py
Description: 
...
from datamodels.dm_indicator import IndicatorSet
...
class IndicatorManager:
    def __init__(self):
        self.indicators_dir = settings.INDS_DIR
...
def load_all_indicators() -> List[IndicatorSet]:
    iset = []
    for fname in os.listdir(settings.INDS_DIR):
        if not fname.endswith(".json"):
            continue
        path = os.path.join(settings.INDS_DIR, fname)
        with open(path, "r") as f:
            data = json.load(f)
            try:
                indicator_set = IndicatorSet(**data)
